...
public class SyringeLoadComponent {
     ...
     * 加载补丁
     */
    public void loadPlug(@NonNull Activity activity, @NonNull String plugPath) {
        if (isGoingToFix(plugPath)) {
            loadedDex.clear();
            doDexInject(activity, plugPath);
        }
    }

    private void doDexInject(@NonNull Activity activity, @NonNull String plugPath) {
